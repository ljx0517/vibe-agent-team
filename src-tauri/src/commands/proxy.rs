...
pub fn apply_proxy_settings(settings: &ProxySettings) {
    ...
    if !settings.enabled {
        ...
        // SAFETY: These environment variables are application-specific proxies
        // and don't affect global process state in a harmful way.
        unsafe {
    ...
    // SAFETY: Setting application-specific proxy environment variables.
    unsafe {
